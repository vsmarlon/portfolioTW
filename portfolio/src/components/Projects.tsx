  ...
  return (
    <section id="projects" className="content-section">
      <div className="editorial-container">
         <div ref={reveal} className="reveal section-heading"><p className="eyebrow">{t('projects.eyebrow')}</p><h2>{t('projects.title')}</h2><p>{t('projects.description')}</p></div>
        <article ref={reveal} className="reveal freebay-flagship card-interactive mt-10 grid gap-8 border-[3px] border-stone-900 bg-[#fffdf8] p-5 dark:border-stone-100 dark:bg-[#131110] md:grid-cols-[1.15fr_0.85fr] md:p-8">
         ...
         <div className="mt-10 grid gap-4">
            {supportingProjects.map((project, index) => <div key={project.title} ref={reveal} className="reveal" style={{ transitionDelay: `${Math.min(index, 4) * 80}ms` }}><a href={project.url} target="_blank" rel="noopener noreferrer" className="card-interactive focus-ring grid gap-4 border-2 border-stone-900/20 bg-[#fffdf8]/60 p-5 dark:border-stone-100/20 dark:bg-white/[0.02] md:grid-cols-[1fr_auto] md:items-center"><div><p className="eyebrow">{project.eyebrow.startsWith('projectsData.') ? t(project.eyebrow) : project.eyebrow}</p><h3 className="mt-2 text-xl font-bold text-stone-900 dark:text-stone-100">{project.title}{project.url.startsWith('http') ? <> <ExternalMark /></> : null}</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-stone-700 dark:text-stone-300">{project.description.startsWith('projectsData.') ? t(project.description) : project.description}</p>{project.details ? <ul className="mt-3 space-y-1.5">{project.details.slice(0, 2).map((detail) => <li key={detail} className="text-xs leading-5 text-stone-600 dark:text-stone-400">· {detail}</li>)}</ul> : null}</div><span className="font-mono text-xs text-stone-600 dark:text-stone-400">{project.tools.join(' · ')}</span></a></div>)}
        </div>
      </div>
    </section>
  );
